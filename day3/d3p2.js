function walk(input) {
  ...
  wire=[];
  for (step of input) {
    direction=step.slice(0,1);
    distance=step.slice(1);

    switch (direction){
      case "R":
        for (i=0;i<distance;i++) {
          wire.push(++currX + "," + currY);
        }
        break;
      case "L":
        for (i=0;i<distance;i++) {
